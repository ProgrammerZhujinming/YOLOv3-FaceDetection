...
class YOLO_V3(nn.Module):
    def __init__(self,class_num=80, factor=0.25):
        ...
        self.bigger_detect = nn.Sequential(
            ...
            CBL(1024, 512, 1, 1, 0),
            CBL(512, 1024, 3, 1, 1),
            CBL(round(1024 * factor), 3 * (5 + class_num), 1, 1, 0, inplace=False, factor=1),
        )
        ...
        self.middle_detect = nn.Sequential(
            ...
            CBL(512, 256, 1, 1, 0),
            CBL(256, 512, 3, 1, 1),
            CBL(round(512 * factor), 3 * (5 + class_num), 1, 1, 0, inplace=False, factor=1),
        )
        ...
        self.small_detect = nn.Sequential(
            ...
            CBL(256, 128, 1, 1, 0),
            CBL(128, 256, 3, 1, 1),
            CBL(round(256 * factor), 3 * (5 + self.class_num), 1, 1, 0, inplace=False, factor=1),
        )
    ...
    def initialize_weights(self, pre_weight_file, isFreezed = True):
        ...
        net_param_dict = torch.load(pre_weight_file, map_location=torch.device("cpu"))
